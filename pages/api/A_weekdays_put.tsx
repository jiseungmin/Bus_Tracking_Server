...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const stationData = data.toObject() as StationData;
      ...
      if (foundSchedule) {
        ...
        // データを更新する
        await CheonanStation.updateOne(
          { _id: stationData._id },
          { $set: { CheonanStation: stationData.CheonanStation } }
        );

        // stationData の更新を data に反映させる必要がある場合
        // 必要な場合、stationData.CheonanStationの要素をdata.CheonanStationにコピーする
        data.CheonanStation = stationData.CheonanStation;
        console.log('data.CheonanStation updated:', data.CheonanStation);
        data = data.CheonanStation;
        console.log('data instanceof CheonanStation:', data instanceof CheonanStation);

        // await data.markModified('CheonanStation');
        await data.save(); // data オブジェクトの保存
